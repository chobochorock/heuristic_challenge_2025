    ...
    def adversarial_search(self, board: GameBoard, time_limit: float) -> Action:
        ...
        * Complete this function to answer the challenge PART IV.
        ...
        def path_finding(state, player) : 
            ...
            while frontier : 
                ...
                if not current_frontier['moves'] == [] :
                    ...
                    except Exception as e: board.debug(f"Exception : {e}")
        ...
        def fence_finding(state, player, opponent_path=None) -> List[BLOCK] :
            ...
            for fence in board.get_applicable_fences(player=player) :
                ...
                try : # 잘 안돌아 가는 것 같음
                    ...
                    # 지나지 못한다면(예외가 발생한다면), 경로를 막는 장벽이므로 합격!
                    blocking_fences.append(BLOCK(player, coord, orient))
        ...
        def utility(state) : 
            ...
            try :    player_path_len   = len(path_finding(state, self.player)) 
            except : player_path_len   = INF
            try :    opponent_path_len = len(path_finding(state, self.opponent))
            except : opponent_path_len = INF
        ...
        def get_actions(state, player) -> List[Action] :
            ...
            try : 
                moves = [path_finding(state, player)[0]] # Nonetype error occured
            except :
                board.set_to_state(state) 
                moves = [MOVE(player, move) for move in board.get_applicable_moves(player)]

            if board.number_of_fences_left(player) > 0 : fences = fence_finding(state, player)
            if fences is None or fences == [] : 
                board.set_to_state(state)
                if board.number_of_fences_left(player) == 0 : fences = []
                else : fences = [
                        BLOCK(player, edge=coord, orientation=orient) 
                        for coord, orient in 
                        sample(board.get_applicable_fences(player), 3)
                    ]

            if moves + fences == [] or moves + fences is None : 
                board.set_to_state(state)
